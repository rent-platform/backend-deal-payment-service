...
- интеграцию с communication-service (системные сообщения в чат)
...
# Интеграции
## Catalog Service
`GET /api/catalog/items/{itemId}/deal-info` — информация о товаре

`GET /api/catalog/items/{itemId}/availability` — календарь доступности

## User Service
`GET /api/users/{userId}/public` — публичный профиль

`PUT /api/internal/users/{userId}/billing` — обновление платёжного профиля

## Communication Service
`POST /api/internal/chats/deal-status` — системное сообщение при смене статуса сделки
...
- Интеграция с communication-service